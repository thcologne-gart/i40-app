    ...
    <v-row id ="row-chapter">
        ...
        <v-col id = "content" md = "10">
          ...
          <div v-else-if="choosedSite !== '' & choosedBuilding === ''">
            <ManagementSite :site="choosedSite" :submodels="submodels" />
          </div>
          <div v-else-if="choosedBuilding !== '' & choosedSite === ''">
            <ManagementBuilding :building="choosedBuilding" :submodels="submodels" :buildings="buildings" />
          </div>
        </v-col>
    </v-row>
</template>
...
<script>
import ManagementSite from '@/components/ems/management/sites/ManagementSite.vue'
import ManagementBuilding from '@/components/ems/management/buildings/ManagementBuilding.vue'

export default {
  components: { ManagementSite, ManagementBuilding },
  props: {
    sites: Array,
    buildings: Array,
    linkToInfos: String
